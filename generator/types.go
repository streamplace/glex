...
func (gen *CodeGenerator) fieldType(fname string, def *lexicon.SchemaDef, optional bool) (string, error) {
	...
	case lexicon.SchemaUnknown:
		...
		default:
			return "", fmt.Errorf("unknown config type: %s", gen.Config.UnknownType)
		}
...
func (gen *CodeGenerator) writeStruct(ft *FlatType, obj *lexicon.SchemaObject) error {
	...
	if _, ok := obj.Properties["$type"]; !ok {
		...
		cborExtra := ""
		if gen.Config.LegacyMode {
			cborExtra = fmt.Sprintf(",const=%s", typeConst)
		}
		fmt.Fprintf(gen.Out, "\tLexiconTypeID string `json:\"$type\"")
		if gen.Config.LegacyMode {
			fmt.Fprintf(gen.Out, " cborgen:\"$type,const=%s\"", typeConst)
		}
		_ = cborExtra
		fmt.Fprintf(gen.Out, "`\n")
	}
	...
	for _, fname := range keys {
		...
		fmt.Fprintf(gen.Out, " `json:\"%s%s\"", fname, omitempty)
		if gen.Config.LegacyMode {
			fmt.Fprintf(gen.Out, " cborgen:\"%s%s\"", fname, omitempty)
		}
		fmt.Fprintf(gen.Out, "`\n")
	}
	fmt.Fprintf(gen.Out, "}\n\n")
...
func (gen *CodeGenerator) writeUnion(ft *FlatType, union *lexicon.SchemaUnion) error {
	...
	// CBOR marshalling of unions: in DaslMode always emit; in legacy mode only
	// for records/subscriptions with specific hacks.
	emitCBOR := gen.Config.DaslMode
	if gen.Config.LegacyMode {
